...
!devChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
        ...
        beforeEach(async () => {
            ...
            const myDynamicSvgNft = await deployments.get("DynamicNFT");
            dynamicSvgNft = await ethers.getContractAt(
                myDynamicSvgNft.abi,
                myDynamicSvgNft.address
            )
        ...
        describe("constructor", () => {
            it("sets starting values correctly", async function () {
                const lowSVG = await dynamicSvgNft.getLowImgURI()
                const highSVG = await dynamicSvgNft.getHighImgURI()
